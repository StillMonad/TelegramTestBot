...
class Shop:
    def __init__(self, bot: TeleBot, db):
        self.call_data = "Лавка студии"
        self.commands = ["магазин", "shop"]
        self.text = "Лавка студии"

        self.markup = self.__make_markup(bot, db)
        self.__init_handlers(bot)

    def __make_markup(self, bot, db):
        self.slider = slider(bot, self.get_goods(db), parent="Главное меню")
